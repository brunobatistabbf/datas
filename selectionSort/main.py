from datetime import datetime
from selectionSort import sort

movimentacoes = 0
comparacoes = 0

start = datetime.now()

arr = open('dadosSelection/dados100milmelhor.txt').read().split(", ")

array, movimentacoes, comparacoes = sort(arr, movimentacoes, comparacoes)

end = datetime.now()

arquivo = open('arquivosSelections/selection100milmelhorcaso.txt', 'x')
arquivo.write("Bruno Batista Ferreira\n")
arquivo.write("SelectionSort")
tempo = str(end - start)
arquivo.write("\n")
arquivo.write("Comparacoes: ")
comparacoesStr = str(comparacoes)
arquivo.write(comparacoesStr)
arquivo.write("\n")
movimentacoesStr = str(movimentacoes)
arquivo.write("Movimentacoes: ")
arquivo.write(movimentacoesStr)
arquivo.write("\n")
arquivo.write(tempo)
arquivo.write("\n")

for i in range(len(array)):
    arquivo.write(array[i] + ", ")